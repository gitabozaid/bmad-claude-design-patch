...
<step n="3.5" goal="Triage Deferred Work">

<action>Read the deferred work backlog: {implementation_artifacts}/deferred-work.md</action>

<action>Filter entries written during this epic's stories. An entry belongs to this epic if its heading contains a story number that matches this epic (e.g., for Epic 3, match `story-3.X` or `3-X-`) OR if its `date:` falls within the epic's active window.</action>

<check if="no entries belong to this epic">
  <output>
Bob (Scrum Master): "No deferred items from Epic {{epic_number}}. Either we fixed everything inline or nothing was legitimately deferrable — both are good."

Charlie (Senior Dev): "Or we cheated. But the story protocol blocks silent skips now, so I'll trust the data."
  </output>
  <action>Skip to step 4.</action>
</check>

<check if="entries belong to this epic">
  <action>Walk each entry one at a time. For each, present:
    - `id`, `severity`, `reason`, `location`
    - `justification`
    - `source` (code-review / pr-review)
  </action>

  <action>For each entry, ask the user to choose:
    - **fix-now** → spawn a small cleanup story in the current sprint OR fix inline if truly small; then mark the entry resolved (move under `## Resolved on {date}` section with a one-line note).
    - **keep-deferred** → rewrite the entry's `date:` field to today's date (staleness marker); keep it in the deferred section. Confirm the reason is still one of the 4 allowed values.
    - **dismiss** → the entry turned out to be noise or got superseded by later changes. Move under `## Resolved on {date}` with reason "dismissed: {why}".
  </action>

  <action>When all entries are walked, produce a summary: X fixed, Y kept, Z dismissed. Append this summary to the retrospective notes.</action>

  <output>
Bob (Scrum Master): "That's {{fix_count}} fixed, {{keep_count}} kept deferred, {{dismiss_count}} dismissed. Clean backlog going into the next epic."
  </output>
</check>
